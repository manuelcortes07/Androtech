...
**`app.py`: 5.301 → 4.861 líneas.** Quedan **~65 rutas** por mover.
...
1. **clientes** — `clientes, nuevo_cliente, editar_cliente, borrar_cliente,
   historial_cliente, exportar_historial_cliente_pdf, exportar_clientes_csv,
   buscar`. Helpers a extraer: `_csv_empresa_header/_csv_pie/_csv_filename_prefix`,
   `paginar` (ya es módulo). Nav: `request.endpoint=='clientes'/'historial_cliente'`.
2. **inventario** — `inventario, nueva_pieza, editar_pieza, eliminar_pieza,
   api_buscar_piezas, agregar_pieza_reparacion, eliminar_pieza_reparacion`.
3. **reparaciones** — `reparaciones, nueva_reparacion, editar_reparacion,
...
4. **facturacion/pagos** — `marcar_reparacion_pagada, publico_pagar, pago_exito,
...
5. **suscripcion** — `signup, suscripcion, suscripcion_bloqueado,
...
6. **admin** — `admin_usuarios, nuevo_usuario, editar_usuario, borrar_usuario,
   ...
   aceptar_solicitud, rechazar_solicitud, borrar_solicitud`.
7. **cuenta/perfil** — `perfil, cambiar_password, cambiar_email,
   cambiar_datos_taller, subir_logo_taller, eliminar_logo_taller`. Nav:
   `url_for('perfil')`.
